...
use PVE::Storage::LunCmd::OviOSiSCSI;
...
my $zfs_unknown_scsi_provider = sub {
    ...
    die "$provider: unknown iscsi provider. Available [comstar, istgt, iet, LIO, OVIOS]";
};
...
my $zfs_get_base = sub {
    ...
    } elsif ($scfg->{iscsiprovider} eq 'OVIOS') {
        return PVE::Storage::LunCmd::OviOSiSCSI::get_base;
    } else {
...
sub zfs_request {
    ...
    if ($lun_cmds->{$method}) {
        if ($scfg->{iscsiprovider} eq 'comstar') {
            ...
            $msg = PVE::Storage::LunCmd::OviOSiSCSI::run_lun_command($scfg, $timeout, $method, @params);
        } else {
            $zfs_unknown_scsi_provider->($scfg->{iscsiprovider});
        }
    } else {
...
sub properties {
    return {
        ...
        debuglogging => {
            description => "enable debug logging for OviOS",
            type => 'boolean',
        },
        comstar_tg => {
